...
export default function FoodMain() {
     ...
     //c1List를 배열로 초기값을 줘야 하나가 아닌 그에 해당하는 여러개가 나올수있음 
     console.log(FoodData);
     let c1 = FoodData.map(item => item['운영주체 분류'])
     c1 = new Set(c1); //c1의 데이터값들 중에서 set으로 중복 없앤게 다시 c1!!
     c1 = [...c1] ; //set타입에서는 map을 더이상 쓸수없기때문에
     // array 로 프로토타입을 배열로 만들어주는 방법.
     
     //c는 말그대로 내가 넣는 어떤값 아무거나지 8비영리민간단체 일수도 3사단법인일수도
     const handleC1 = (c) => {   
          console.log(c);
          let tm = FoodData.filter(item=> item['운영주체 분류'] === c )
          //제이슨에서 운영주체분류 값이 c와 같은 것만 골라내온 배열이 tm 
          //c는 3.사단법인 이면 그 값인것들을 다 추려내면 tm임 사단법인 값을 가지는 모든걸 배열
          console.log(tm)
     }
